...
async function configure(settings) { const stored = await evaluate(`(async () => { await chrome.storage.local.set({ 'aitools.settings': ${JSON.stringify(settings)} }); return (await chrome.storage.local.get('aitools.settings'))['aitools.settings']; })()`); if (stored?.newTabDestination !== settings.newTabDestination || stored?.newTabSearchEngine !== settings.newTabSearchEngine || (Object.hasOwn(settings, 'newTabCustomUrl') && stored?.newTabCustomUrl !== settings.newTabCustomUrl)) throw new Error('La préférence de nouvel onglet n’a pas été enregistrée avant le test.'); }
...
await configure({ newTabDestination: 'custom', newTabSearchEngine: 'google', newTabCustomUrl: 'https://example.com/personal-dashboard', featureFlags: {} });
const customUrls = await openNewTab();
await configure({ newTabDestination: 'custom', newTabSearchEngine: 'google', newTabCustomUrl: 'https://example.com/personal-dashboard', featureFlags: { 'newtab.custom': false } });
const disabledCustomUrls = await openNewTab();
await configure({ newTabDestination: 'dashboard', newTabSearchEngine: 'google', featureFlags: {} });
socket.close();
const results = { dashboardUrls, searchUrls, nativeUrls, disabledDashboardUrls, disabledSearchUrls, customUrls, disabledCustomUrls };
...
if (!customUrls.some((url) => url.startsWith('https://example.com/personal-dashboard'))) throw new Error('La destination personnalisée validée ne redirige pas correctement.');
if (!disabledCustomUrls.includes('chrome-search://local-ntp/local-ntp.html')) throw new Error('Une destination personnalisée désactivée ne bascule pas vers le repli Chrome natif.');
