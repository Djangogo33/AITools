...
-- Sauvegarde restaurable v8.4 : données locales personnelles non représentées par
-- les tables métier ci-dessus (captures, focus, règles, recherche et préférences DND).
-- Les sessions OAuth, vérificateurs PKCE, cache de compte et diagnostics restent exclusivement locaux.
...
create policy "user_backups_select_own" on public.user_backups for select to authenticated using ((select auth.uid()) = user_id);
drop policy if exists "user_backups_insert_own" on public.user_backups;
create policy "user_backups_insert_own" on public.user_backups for insert to authenticated with check ((select auth.uid()) = user_id);
drop policy if exists "user_backups_update_own" on public.user_backups;
create policy "user_backups_update_own" on public.user_backups for update to authenticated using ((select auth.uid()) = user_id) with check ((select auth.uid()) = user_id);
drop policy if exists "user_backups_delete_own" on public.user_backups;
create policy "user_backups_delete_own" on public.user_backups for delete to authenticated using ((select auth.uid()) = user_id);
drop trigger if exists user_backups_set_updated_at on public.user_backups;
create trigger user_backups_set_updated_at before update on public.user_backups for each row execute procedure public.set_updated_at();
