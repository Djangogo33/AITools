...
// Load mode and set on HTML for ai-injected.js to read
// THEN inject page script (must happen after mode is set)
(async () => {
  try {
    const data = await new Promise((resolve) => {
      chrome.storage.local.get('aiMode', resolve);
    });
    const aiMode = data.aiMode || 'free';
    document.documentElement.setAttribute('data-ai-mode', aiMode);
    console.log('[Content] 📌 Mode set on HTML:', aiMode);
  } catch (error) {
    console.log('[Content] Mode load failed, using default "free"');
    document.documentElement.setAttribute('data-ai-mode', 'free');
  }
  
  // NOW inject script (after mode attribute is set)
...
// ============================================================================
// REMOVED DUPLICATE - See line 736 for main version
...
function debugDOM() {
  ...
  console.log('[DEBUG] .mw-parser-output exists:', !!document.querySelector('.mw-parser-output'));
  console.log('[DEBUG] #mw-content-text exists:', !!document.querySelector('#mw-content-text'));
  console.log('[DEBUG] <article> exists:', !!document.querySelector('article'));
  console.log('[DEBUG] <main> exists:', !!document.querySelector('main'));
  
  const parserOutput = document.querySelector('.mw-parser-output');
  if (parserOutput) {
    console.log('[DEBUG] .mw-parser-output text length:', parserOutput.innerText.trim().length);
    console.log('[DEBUG] .mw-parser-output first 200 chars:', parserOutput.innerText.trim().substring(0, 200));
  }
  
  const mainContent = document.querySelector('#mw-content-text');
  if (mainContent) {
    console.log('[DEBUG] #mw-content-text exists, text length:', mainContent.innerText.trim().length);
  }
  ...
  console.log('[DEBUG] Total <p> tags:', allPs.length);
  const largePs = Array.from(allPs).filter(p => p.innerText.trim().length > 100);
  console.log('[DEBUG] <p> tags with > 100 chars:', largePs.length);
  if (largePs.length > 0) {
    console.log('[DEBUG] First large <p>:', largePs[0].innerText.trim().substring(0, 150));
  }
}
...
function extractRelevantPageText() {
  ...
  for (const element of candidates) {
    ...
    text = text.trim();
    console.log(`[Extractor] Trying selector, got ${text.length} chars`);
    
    if (text.length < 100) continue; // Too small
    ...
    text = text
      ...
      .filter(line => {
        ...
        if (trimmed.match(/^\[\d+\]$/)) return false; // Just [1] [2]
        if (trimmed === 'Conteúdos' || trimmed === 'Contents') return false;
        return true;
      })
      .join('\n');
    
    if (text.length > 100) {
      console.log(`[Extractor] ✅ Found ${text.length} chars from element`);
      return text.substring(0, 2000);
    }
  }
  
  // Last resort: clean body
  console.log('[Extractor] Trying document.body fallback');
  ...
  text = text
    ...
    .filter(line => {
      ...
      if (t.match(/^(Conteúdos|Contents|Navbar|Menu|Navigation)/i)) return false;
      return true;
    })
    .join('\n')
    .trim();
  
  console.log(`[Extractor] Body fallback: ${text.length} chars`);
  return text.substring(0, 2000);
}
...
async function generateSummaryWithAI(text, length = 35) {
  ...
  try {
    ...
    console.log('[Summarizer] 📡 Attempting AI summarization...');
    ...
    if (!result) {
      ...
      console.log('[Summarizer] 💡 Using local heuristic extraction');
      return betterSummarize(text, length);
    }
...
async function generateTranslationWithAI(text, sourceLang, targetLang) {
  ...
  try {
    // MyMemory has strict 500 char limit
    const textForAI = text.substring(0, 500);
    ...
    if (result) {
      ...
      chrome.storage.local.get('aiMode', (data) => {
        ...
        if (mode === 'smart') {
          console.log('[Translator] 💡 Activate Gemini Nano (chrome://flags) or add OpenAI key');
        } else {
          console.log('[Translator] 💡 Mode Gratuit: MyMemory API unavailable');
        }
      });
      return null;
    }
...
// ============================================================================
// TRANSLATE TEXT IN CHUNKS - Handle large texts
// ============================================================================
async function translateInChunks(text, sourceLang, targetLang) {
  const CHUNK_SIZE = 400; // Safe limit for MyMemory
  const chunks = [];
  ...
  for (let i = 0; i < chunks.length; i++) {
    ...
    try {
      const result = await window.aiService.callLibreTranslate(chunk, targetLang, sourceLang);
      if (result) {
        translatedChunks.push(result);
        console.log(`[Translator] Chunk ${i+1}/${chunks.length} ✅`);
      } else {
        // Fallback to original if translation fails
        translatedChunks.push(chunk);
        console.log(`[Translator] Chunk ${i+1}/${chunks.length} ⚠️ (fallback to original)`);
      }
    } catch (e) {
      console.error(`[Translator] Chunk ${i+1} error:`, e.message);
      translatedChunks.push(chunk);
    }
  ...
  console.log(`[Translator] Total translation: ${fullTranslation.length} chars`);
  return fullTranslation;
}

// ============================================================================
// OVERLAY TRANSLATION - Replace text in place with overlay
// ============================================================================
function showTranslationOverlay(translatedText, sourceLang, targetLang) {
  // Get the .mw-parser-output element
  const contentElement = document.querySelector('.mw-parser-output');
  if (!contentElement) {
    alert('❌ Cannot find content element');
    return;
  }
  
  const langNames = {
    pt: 'Portugais', fr: 'Français', en: 'Anglais', es: 'Espagnol',
    de: 'Allemand', it: 'Italien', ja: 'Japonais', zh: 'Chinois'
  };
  
  // Store original HTML for undo
  const originalHTML = contentElement.innerHTML;
  
  // Create overlay div that will replace the content
  ...
  // Add undo button at top
  const undoBar = document.createElement('div');
  undoBar.style.cssText = `
    background: #FFC107; color: #333; padding: 12px 16px;
    margin-bottom: 16px; border-radius: 6px; display: flex;
    justify-content: space-between; align-items: center;
    font-weight: 600; font-size: 14px;
  `;
  undoBar.innerHTML = `
    <span>✅ Traduction appliquée (${langNames[targetLang] || targetLang})</span>
    <button style="background: #FF5252; color: white; border: none; padding: 8px 16px;
      border-radius: 4px; cursor: pointer; font-size: 12px; font-weight: 600;">
      ↶ Annuler
    </button>
  `;
  
  contentElement.appendChild(undoBar);
  ...
  undoBar.querySelector('button').addEventListener('click', () => {
    ...
    console.log('[Translator] Translation reverted');
  });
  
  console.log('[Translator] ✅ Translation applied to page');
}
...
function addTranslatorButton(sourceLang, targetLang) {
  ...
  btn.addEventListener('click', async (e) => {
    if (!e.target.closest('.aitools-close-btn')) {
      ...
      const relevantText = extractRelevantPageText();
      
      if (!relevantText || relevantText.length < 50) {
        alert('❌ Impossible d\'extraire le contenu de cette page');
        textSpan.textContent = '🌐 Traduire';
        btn.disabled = false;
        btn.style.opacity = '1';
        return;
      ...
      console.log(`[Translator] Translating ${relevantText.length} chars in chunks...`);
      
      // Translate in chunks
      const translated = await translateInChunks(relevantText, sourceLang, targetLang);
      
      if (translated && translated.trim().length > 100) {
        console.log('[Translator] ✅ Translation complete, applying to page');
        showTranslationOverlay(translated, sourceLang, targetLang);
      } else {
        console.warn('[Translator] ⚠️ Translation failed');
        alert('❌ Traduction échouée');
      }
...
chrome.runtime.onMessage.addListener((req, sender, sendResponse) => {
  if (req.action === 'checkPromptAPI') {
    // In FREE mode, never check for Prompt API
    const aiMode = document.documentElement.getAttribute('data-ai-mode') || 'free';
    if (aiMode === 'free') {
      console.log('[Content] FREE mode - skipping Prompt API check');
      sendResponse({
        apiAvailable: false,
        reason: 'FREE mode - no Prompt API check'
      });
      return true;
    }
    
    // SMART mode: check if AIService thinks the API is available
