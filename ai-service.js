...
class AIService {
  ...
  async callLibreTranslate(text, targetLang = 'fr') {
    try {
      ...
      const textForAPI = text.substring(0, 2000);
      const encoded = encodeURIComponent(textForAPI);
      
      console.log(`[AIService] 📝 Text to translate (${textForAPI.length} chars), target: ${targetCode}`);
      
      // MyMemory API (GET request = no CORS issues)
      const response = await fetch(
        `https://api.mymemory.translated.net/get?q=${encoded}&langpair=auto|${targetCode}`
      );
