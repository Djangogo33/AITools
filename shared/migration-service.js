...
export async function migrateLocalData() {
  const stored = await chrome.storage.local.get([SCHEMA_VERSION_KEY, 'aitools.tasks', 'aitools.settings', 'aitools.capture-inbox', 'aitools.workspaces']);
  ...
  if (fromVersion < 7) {
    ...
    if (Array.isArray(stored['aitools.capture-inbox'])) updates['aitools.capture-inbox'] = stored['aitools.capture-inbox'].map((capture) => ({ ...capture, processedAt: validDate(capture?.processedAt) ? capture.processedAt : null, dismissedAt: validDate(capture?.dismissedAt) ? capture.dismissedAt : null }));
